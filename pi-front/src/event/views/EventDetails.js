...
function EventDetails(props) {
  ...
  const initialState = { eventName: "", fullName: "", userEmail: "", guestSize: "", phone: "", participateAt: "" };
                  ...
                  <div class="iq-card">
                    <div class="iq-card-body">
                      <form onSubmit={Handelsubmit} id="form-wizard1" class="text-center mt-4">
                        ...
                        <button type="submit" class="btn iq-bg-danger">
                          cancle
                        </button>
                      </form>
                    </div>
              ...
              <div class="col-md-8">
                <div class="iq-card iq-card-block iq-card-stretch iq-card-height">
                  <div class="iq-card-body p-0  ">
                    ...
                    <div>
                      <p class="card-text">
                        <i class="bi bi-pass-fill"></i>description :{data.description} <br></br>
                        <i class="bi bi-pin-map-fill"></i>location : {data.location} <br></br>
                        <i class="bi bi-person-fill"></i>organizer : {data.organizer} <br></br>
                        <i class="bi bi-balloon-heart-fill"></i>likes : {data.nbLikes}{" "}
                      </p>
                    </div>
                    {likeee.length == 0 ? (
                      <Button onClick={likeeventt} color="primary" className="float-left">
                        <FaThumbsUp />
                        Like
                      </Button>
                    ) : (
                      <Button onClick={dislikee} color="primary" className="float-left">
                        <FaThumbsDown />
                        dislikee
                      </Button>
                    )}
                    ...
                    <p>
                      {" "}
                      <i class="bi bi-volume-up-fill"></i>Read for me{" "}
                    </p>
                    <Button class="btn btn-primary btn-lg" className="btn-round" color="info" size="md" onClick={() => synth.speak(new SpeechSynthesisUtterance(data.description))}>
                      <i class="bi bi-headphones"></i>
                    </Button>
                    <Button class="btn btn-primary btn-lg" className="btn-round" color="warning" size="md" onClick={() => synth.pause()}>
                      <i class="bi bi-pause-circle"></i>
                    </Button>
                    <Button class="btn btn-primary btn-lg" className="btn-round" color="success" size="md" onClick={() => synth.resume()}>
                      <i class="bi bi-fast-forward-circle"></i>
                    </Button>
                    <Button class="btn btn-primary btn-lg" className="btn-round" color="danger" size="md" onClick={() => synth.cancel()}>
                      <i class="bi bi-power"></i>
                    </Button>
                    <p>
                      {" "}
                      <i class="bi bi-cloud-sun-fill"></i>check weather
                    </p>
                    <button type="submit" class="btn btn-dark">
                      <i class="bi bi-cloud-sun"></i>
                      <NavLink to={`/takss`}>chek weather</NavLink>
                    </button>
                  </div>
                </div>
              </div>
